...
- 依存関係の自動セットアップ、cobra CLI骨格、Phase 2〜4（domain／detector／encoder／evaluator／engine.bsearch／orchestrator統合）、Phase 5（bubbletea TUIダッシュボード＋ログ方針確定）、Phase 6（音声3モード＋README整備・配布レイアウト統一）、Phase 7（TUIウィザード化: setup→run→summaryの3ステージ・起動モード自動判定・--headless）、配布物整備（LICENSE・THIRD-PARTY-NOTICES全文埋め込み・`go run ./cmd/engram-package` によるZip化・解凍先からの実行検証済み）、Phase 8（TUIウィザードのパラメータ実値選択化）、起動導線の確定（cobra mousetrap無効化によるダブルクリック→ウィザード直行・裸起動経路のdecideLaunch一本化〔--headless/--tui排他を全経路で適用〕・裸起動エラー時の[Enter]待ち・フラグ値のウィザード初期値反映）、配布追跡性の確保（`--version`＝Zip名と同一バージョンをldflags埋め込み・cli/integration_test.goによる実バイナリ起動フロー検証）、CI構築（`.github/workflows/ci.yml`: windows-latest上のDev-CI Parity。engram-setup導入物キャッシュ・gofmt/vetゲート・フルテスト・**race検出はCIでのみ実施**。リモート登録後の初回pushで稼働）、評価プロファイル制と出力解像度指定（`domain.EvalProfile`: vmaf_v1.0.16_3d0h@1080p / vmaf_4k_v0.6.1@2160p の2段選択。`--eval-profile`/`--out-res`/`--enc-args`。フォールバック廃止=フェイルファスト。パラメータ表示はffmpeg実名統一。未指定out-resは入力実寸へ自動解決）まで **実装完了・動作確認済み**（機能開発はいったん完了）。**現在は「システム全体の改善フェーズ」（2026-08-23開始）**: コード修正・バグハント反復・リファクタリング・品質ゲート強化・検証拡充（memo.md末尾チェックリストの消化）が主作業。方針と制約は memo.md 冒頭「開発フェーズ宣言」を参照。実測知見は memo.md「依存ツール」節、バグハント記録は同「設計強化の記録」節を参照。改善フェーズ実績: バグハント3ラウンド（不具合7件修正）・staticcheck全解消・Fuzz5ターゲット・precheck層新設。
