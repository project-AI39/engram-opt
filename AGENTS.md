...
- 依存関係の自動セットアップ、cobra CLI骨格、Phase 2（domain＋detector）、Phase 3（encoder／evaluator／engine.bsearch、`optimize --shot N` で単一シーン二分探索）まで実装完了・動作確認済み。次ステップはPhase 4（orchestrator統合： 全シーン逐次処理→concat）。開発フェーズは memo.md「モジュール構成（実装方針）」を参照。libvmaf評価の時間基準落とし穴など実測知見は memo.md「依存ツール」節に記録済み。
