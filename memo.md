...
internal/
  ...
  ├── media/            # ffprobe共通取得（フレームレート・キーフレームアンカー）
...
- **チャンク抽出の高速化（キーフレームアンカー事前シーク・2026-08）**: StartFrame>0の試行では、ffprobeパケット走査（メタデータのみ・`-read_intervals`で区間限定）によりS以前最後のキーフレームAを実測pts付きで検出し、`-ss <Aのpts>` 入力シーク後にselect範囲を `between(n, S-A, E-A)` へ平行移動する。デコード開始点を同期点（IDR）に限定するため、出力はフルデコードとビット同一（イントラ起点で参照連鎖が完全リセットされる）。旧「-ss禁止」規約が防いでいた浮動小数点推定の丸め問題は、実在パケットの厳密なptsを使うことで発生しない。
  - 前提はCFR（既存select方式と同一。VFRは引き続き想定外）
...
- 意図的に露出しない項目（構造的固定点）: GOP長・select整数フレーム抽出（`-ss`はキーフレームアンカー目的の内部使用に限定。ユーザー露出なし）・settb/setpts正規化・shortest=eof_action=endall・評価時1920x1080強制スケール・音声自動ビットレート表・av-scenechange閾値・シーン逐次処理。
...
- **キーフレームアンカー事前シークの効果（2026-08実測）**: 60秒ソースのフレーム1500地点（50秒目）シーン1本のエンコード試行が 0.55s → 0.20s（約2.8倍）。削減量は開始位置Sに比例し、長尺動画ほど効果が増大する。等価性は「アンカー経路⇔フルデコード経路」のデコード済み生フレームSHA256一致（encoder）・VMAF全統計完全一致（evaluator）の統合テストで恒久担保。
...
## 8.1 試行デコード量のスケーラビリティ（2026-08解決）

かつては各CRF試行・各評価が動画先頭から全デコードし、総デコード量 ≈ 試行数 × Σ(前方フレーム数) となる問題を抱えていた（2時間映画で数十時間オーダーの純デコード）。**キーフレームアンカー事前シーク**（§4.2参照）により、各試行のデコードは「アンカー残差＋シーン長」へ短縮された。代替案のシーン単位All-Intraメザニン生成は、中間ディスク書き出しがメモリパイプ方針と衝突するため不採用。等価性は「アンカーシーク経路とフルデコード経路の出力がバイト一致する」統合テストで恒久担保している。
