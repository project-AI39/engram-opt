...
# 開発フェーズ宣言（2026-08-23決定）

- **機能開発はいったん完了。これより「システム全体の改善フェーズ」へ移行する。**
- 改善フェーズの活動範囲:
  * **コード修正**: バグ修正・不具合対応（バグハントは実施済みの手法を反復: testdata＋生成クリップによるアドバーサリテスト）
  * **リファクタリング**: 責務境界・命名・重複排除・API表面の最小化（既存方針: フェイルファスト統一・コード↔テスト1:1対応を維持・強化する方向でのみ行う）
  * **品質ゲート**: gofmt / go vet / 全テスト / `go run ./cmd/engram-package` のゲートは全変更で必須（従来どおり最終門番）
  * **検証の拡充**: memo.md末尾チェックリスト（長時間動画・hevc・audio 4モード×bit depth・中断耐性等）の消化も本フェーズの作業に含める
- 制約（改善フェーズ中も変わらない固定点）:
  * エンコード仕様・評価プロファイル制・起動モード判定などの「動作仕様の固定点」を勝手に変更しない。変更が必要なら memo.md を先に更新してから着手する
  * 新機能の追加は原則しない（既存機能の堅牢化・整理が主目的）
...
* 改善実装:
  ...
  * packaging tmpリセットを resetStagingTmp ヘルパー化＋単体テスト化。
  * READMEトラブルシューティングへ新エラー行を追記、AV1 preset行の旧記述(名称可)を修正。
...
攻撃対象: 複数音声/字幕ストリーム・単一フレーム・コンテナ互換・enc-argsガード回避
（=形式/二重ダッシュ/非オプショントークン）・--log-file異常系・同一入力の並行実行・
out≡logfile同一パス。

* **問題なし（堅牢性確認）**:
  * enc-argsガード: "-crf=20" / "--crf" の変形もすべて捕捉。非オプション値(foo bar)は
    試行でfail-fastし迷子ファイルも生成されず。
  * 並行実行: 同一tmpRootで2プロセス同時走行→両方exit 0・jobDir(PID接尾辞)分離・自己清掃。
  * --log-fileがディレクトリ/ロック対象でも起動時または明確なエラーで即中断。
...
* **改善候補 → 実装済み（3718efe・cli/precheck.go新設）**:
  ...
  * 出力拡張子不正(.txt等) → mkv/mp4/webm/mov以外は起動時エラー（探索前に拒否）。
  * --out≡--log-file / log-file≡input の衝突 → 起動時検出。
  * 入力不存在/ディレクトリも os.Stat で分離表示。
  * 出力先ディレクトリ自動作成はREADMEのコマンドリファレンスへ明記。
### バグハント実施記録（2026-08-23 / testdata＋生成クリップでアドバーサリテスト）

攻撃対象: フラグ検証20件・パス/出力先8件・入力エッジ5種（音声なし/静止画/奇数解像度/
超短尺12f/偽装・空ファイル）・実機再実行。発見と対応:
...
正常系の裏取り: 静止画(単一シーン)・超短尺(12フレーム)・相対パス・上書き警告・
ロック済み出力(rename失敗の明確なエラー)はすべて期待どおり。
### エラー処理方針：フェイルファスト統一（2026-08決定）

* **原則**: 暗黙の代替・静かな縮退は禁止。失敗・未知値・契約違反は即座にエラー（プログラマバグはpanic）で表面化させる。ユーザー指定と異なる動作への「親切な」切替は行わない。
* 実装済みの強化点:
  * 評価モデル/評価解像度: プロファイル制で未知名は即エラー（フォールバック廃止済み・`evaluator/libvmaf`冒頭doc参照）。
  * `--preset`: 未知名はエラー（旧実装の黙って medium 相当への置換は廃止済み）。
  * `--metric`: パースは実値3種のみ（旧表記エイリアス廃止済み）。加えて `QualityMetrics.Score` は正規化済み値のみ受容する契約へ変更し、未知/ゼロ値は panic（防御フォールバック廃止）。呼び出し側は必ず `EffectiveMetric()` の結果を渡す。
  * `--tui` をリダイレクト先で実行: 平文へ黙って切替えず `ui.ErrNoTTY` をそのままエラー返却（起動モード分岐は cli 側 decideLaunch が事前に行う）。
  * tmp掃除 `sweepStaleJobs`: ReadDir 失敗も警告ログ必須（完全な黙殺禁止）。チャンク後始末 `cleanupExcept` も同様に失敗時ログ。
* **「フォールバック」と呼んでよいのは環境適合のみ**:
  * `toolbin` レイアウト判定（配布=本体隣 `bin/` ⇄ 開発=リポジトリ `build/`）。実行時の最終解決はバイナリ欠落でフェイルファスト。
  * lipgloss カラー能力への自動適応（表示装飾のみで挙動に影響なし）。
* **仕様としての例外**（黙ってではない・常にログまたは報告付き）:
  * MinCRF ベストエフォート採用 / 単一点範囲CRF採用: 目標未達を警告ログ＋`MetTarget=false`報告のうえ採用する合否仕様。
* **ゼロ値の扱い**: 「未指定→既定値」は正規化と呼び、フォールバックと区別する（`SearchConfig.Effective*`、ウィザード空欄=入力解像度扱い 等。いずれも文書化された既定値であり、失敗隠蔽ではない）。
...
- [x] **Codec 3種**: h264(第2〜4回)/av1(第4回)/hevc(2026-08-24・ultrafast実機) いずれも完走・出力確認済み
- [x] **Audio 4モード**: copy/opus/aac/none をhevc実機で検証（ffprobeで出力音声codec確認）。音声なし入力×opus/aacは起動時エラー化済み
- [ ] **Bit Depth 2種**: 10bitは全検証の既定。**8bitはウィザード専用項目のためCLI経由E2E不可**（pixFmtFor単体テストとdomain.Validateで担保。実TUIでの確認は次回手動）
- [x] **連続稼働**: 本日30回超の連続実行で成功時破棄・失敗時残置(72h sweep対象)・並行2プロセス分離を実機確認
- [ ] **中断耐性**: Ctrl+C 中断時に子プロセス（ffmpeg等）が残らないこと
- [x] **ログ**: --log-file併用はヘッドレス全検証で二重化確認。TUI表示中の二重化はLogMirror実装済み（実TUI確認は次回手動）
...
- 結果: **全4本 exit 0・計190試行（47/51/51/41）＝第2回と試行数まで完全一致**
- **出力は4本すべて旧フィクスチャとbyteサイズ一致**（480p=4613356 / 720p=3866933 /
  1080p=6155625 / 4K=9880140）→ リファクタリング群は探索結果に一切影響していない
- 補足検証: 4Kを `--eval-profile vmaf_4k_v0.6.1` でも実施 → exit 0・8/8達成・10,746,849 bytes
  （hd1080評価比 +8.8%）。4Kネイティブモデルは同一画質基準でより高いビットレートを要求する
  ためで、4K配信の品質保証にはこちらを使用する方針（README固定点表どおり）の裏取り
- 到達率: 480pのみ7/8（MinCRFベストエフォート1シーン＝仕様どおり）、他3本は8/8

### 第4回実行記録（2026-08-23 / testdata×AV1 preset 6マトリクス）

- 条件: 第3回と同一4入力を `--codec av1 --preset 6` で実行。4Kのみ
  `--eval-profile vmaf_4k_v0.6.1`（4K配信方針どおり）。出力は全本 `av1/yuv420p10le` をffprobe確認
- 結果: **全4本 exit 0**・試行 37/32/30/14（h264比で大幅減＝低CRF側へ速やかに収束）
  - 480p: 5/8達成・-13.7%（h264は7/8・-7.0%）→ 未達3シーンはMinCRF15でもVMAF95未満の難素材
  - 720p(VP9源): 6/8達成・+18.9%（h264 +47.1%）→ VP9源への追撃には依然ビットが必要だが半減
  - 1080p: 8/8達成・**-22.3%**（h264 +51.7%）→ 同一画質で圧縮率が逆転
  - 4K(4Kモデル評価): 8/8達成・**-56.7%**（h264 -28.6%）→ アーカイブ用途の効果大
- 処理時間: 80〜224秒（h264実行比で同等〜高速。SVT-AV1 preset6はこの規模では十分実用的）
- 知見: VMAF95固定でもAV1は「サイズ増」ゾーンが大幅に縮小。VP9/高効率ソース以外は
  縮小が見込める。チェックリスト「Codec 3種」のうち h264/av1 を完了（hevcは次回）
