...
* **合否基準指標は選択可能**（2026-08〜）: `harmonic`（既定・従来固定仕様）/ `mean` / `min`。CLIは `--metric`、ウィザードは Metric フィールドで選択。
  - harmonic: 調和平均。低スコア帧のペナルティが最大（知覚的代理として最も保守的）
...
- **評価プロファイル制**: アルゴリズム×評価解像度をセットで指定（domain.EvalProfile）。
  vmaf-hd1080=vmaf_v1.0.16_3d0h@1920x1080（既定）/ vmaf-uhd4k=vmaf_4k_v0.6.1@3840x2160。
