...
* **改善候補（黙って落とす系・未着手）**:
  * 複数音声トラックの2本目以降がcopy/opus問わず無言で欠落（map 1:a:0固定）。
    少なくとも警告ログ、将来的には--audio-track選択 or 明示エラー。
  * 字幕ストリートも同様に無言欠落（mkv入力時に顕在化）。
  * 単一フレーム入力でav-scenechangeの内部エラー文がそのまま表面化
    （「no valid frame_count」。事前検証で分かりやすい拒否へ）。
  * 出力拡張子不正(.txt等)が探索完了後の最終muxで初めて失敗する（早期バリデーション候補）。
  * --out≡--log-file同一パスで最終renameが失敗する（起動時重複チェック候補）。
...
* **[改善候補・未着手]**: 存在しない/偽装/空入力が同一のffprobe-dimsエラーになる
