...
| `--eval-profile <p>` | `hd1080` | **評価プロファイル**（アルゴリズム×評価解像度のセット）: `hd1080`=3d0h@1080p / `uhd4k`=4K専用モデル@2160p。4K配信の品質保証には `uhd4k` を推奨（1080p縮小評価はネイティブ4Kの劣化を見逃すため）。失敗時の暗黙フォールバックなし |
