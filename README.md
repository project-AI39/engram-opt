...
| `--eval-profile <p>` | `vmaf-hd1080` | **評価プロファイル**（アルゴリズム×評価解像度のセット）: `vmaf-hd1080`=3d0h@1080p / `vmaf-uhd4k`=4K専用モデル@2160p。4K配信の品質保証には `vmaf-uhd4k` を推奨（1080p縮小評価はネイティブ4Kの劣化を見逃すため）。失敗時の暗黙フォールバックなし |
| `--out-res <res>` | `native` | 出力解像度: `native`（ソース維持）/ `<偶数>x<偶数>`（例: `1280x720`）。シーン検出は元解像度で行う |
...
| VMAFモデル | 内蔵 `vmaf_v1.0.16_3d0h`（失敗時 `vmaf_v0.6.1neg` にフォールバック） |
| 評価解像度 | 常に1920x1080へリサイズして評価 |
